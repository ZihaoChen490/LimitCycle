...
class FT_model(Module):
    ...
    def ecoder(self, X):
        ...
        for i in range(2000):
            ...
            ecoder_loss = torch.sum(torch.square(X - reconstructed_signal))
            ecoder_loss.backward(retain_graph=True)
            optimizer.step()
            if i % 100 == 1:
                print('In encoder, epoch', str(i), 'with loss ', ecoder_loss)
                if ecoder_loss.detach().numpy() < 1e-6:
                    return y_pred
